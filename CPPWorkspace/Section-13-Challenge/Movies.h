#ifndef MOVIES_H
#define MOVIES_H

#include <vector>
#include <string>
#include "Movie.h"

class Movies
{
private:
    std::vector<Movie> movies;
public:
    Movies();
    ~Movies();
    
    
    /*********************************************************************
     * add_movie expects the name of the movie, rating, and watched count
     * 
     * It will search the movies vector to see if a movie object already
     * exists with the same name.
     * 
     * If it does then return false since the movie already exists.
     * Other wise, create a movie object from the provided information
     * and add that movie object to the movies vector and return true.
     * ******************************************************************/
    bool add_movie(std::string name, std::string rating, int watched);
    
    
    /*********************************************************************
     * increment_watched expects the name of the move to increment the
     * watched count.
     * 
     * It will search the movies vector to see if a movie object already 
     * exists with the same name.
     * 
     * If it does then increment that object's watch by 1 and return true.
     * 
     * Otherwise, return false since then no movies object with the movie
     * name provided exists to increment.
     * *******************************************************************/
     bool increment_watched(std::string name);
     
     
     /*********************************************************************
      display
       * 
       * display all movie objects in the movies vector.
       * For each movie call the movie.display method so
       * the movie object displays itself.
       * *****************************************************************/
       void display() const;
};

#endif // MOVIES_H
